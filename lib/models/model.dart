...
class FSOSTrackerData {
  FSOSTrackerData(this.identificator, this.latitude, this.longitude,
...
// const trackerDataPackageLength = 21; //bytes, defined in FSOS protocol
// String myName = '';
// int countOfTrackers = 0;
// int headerEndIndex = 0;
// List<FSOSTrackerData> trackersDataList = [];
...
//     _parseFSOSDATA(ValidatedData).forEach((new_entry) {
...
//   try {
//     findA = BLEDataStream.indexOf(nameFSOS[0].codeUnitAt(0)); //find "A"
//     if (findA != -1) {
//       String trackerName = ''; // read all word
//       for (final c in BLEDataStream.getRange(findA, findA + nameFSOS.length))
//         trackerName += String.fromCharCode(c);

//       if (trackerName == nameFSOS) isFSOSFind = true; // check word "FSOS"
//     } else
//       return result;

//     if (isFSOSFind) {
//       //validate protocol
//       if (BLEDataStream[findA + nameFSOS.length] != 0xAA) //dec 170
//         isFSOSFind = false; //check BLE protocol version, 1 byte

//       myNameLength = BLEDataStream[findA + nameFSOS.length + 1];

//       if (BLEDataStream[findA + nameFSOS.length + 2] != 0x01)
//         isFSOSFind = false; //check start of heading, 0x01, 1 byte

//       //Find my tracker name
//       final myNameStartIndex = findA + nameFSOS.length + 3;
...
//   if (isFSOSFind) {
//     print(
//         "NEW FSOS BLE PACKET, MY NAME IS ${myName}, ${countOfTrackers} trackers in view");
//   }

//   // Congratulations! Half done! Then navigation data check

//   if (isFSOSFind && (countOfTrackers != 0)) {
//     // Length check
...
// List<FSOSTrackerData> _parseFSOSDATA(List<List<int>> data) {
//   List<FSOSTrackerData> result = [];
//   int count = 0;
...
//       final FSOSTrackerData new_tracker = FSOSTrackerData(
